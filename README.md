...
- `prompt` - Generates a response from the LLM. Further information on this functionality is described above. You can generate a response in the style of of a file from your server's directory by using the `-f` command flag and specifying a file: `$prompt -f dracula`. By default, this command will invoke "reasoning" responses from the LLM. To instead invoke "chat completion" responses, you can use the `-c` command flag: `$prompt -c What is the answer to Life, the Universe, and Everything?`. This command can be invoked with the aliases `$chat` and `$promt`.
...
### Terminal
- `cat` - Sends the entire contents of a given file from the server's directory. You can prefix the content with line numbers by using the `-n` command flag: `$cat -n dracula`. Alternatively, you can number just the nonblank lines with the `-b` command flag: `$cat -b dracula`. This command has pipeline support.
...
- `grep` - Sends all lines from a given file from the server's directory that match a given search string. The search string supports Regular Expressions. For example, to find lines that contain a simple URL for a file named "websites": `$grep websites www.\w+.com`. This command has pipeline support.
...
- `nl` - Sends the entire contents of a given file from the server's directory with prefixed line numbers. You can specify which lines to number by using the `-b` command flag and specifying one of the following style options: `a` number all lines, `n` number no lines, `t` number only nonempty lines. Use the `-i` command flag to specify a line number increment value: `$nl -i 3 dracula`. You can specify the format of inserted line numbers by using this `-n` command flag and specifying one of the following format options: `ln` left justified, `rn` right justified, `rz` right justified with leading zeros. You can specify the string used to separate line numbers from line content with the `-s` command flag: `$nl -s " | " dracula`. You can specify the starting line number with the `-v` command flag: `$nl -v 67 dracula`. You can specify the column width of the line numbers with the `-w` command flag: `$nl -w 2 dracula`. This command has pipeline support.
