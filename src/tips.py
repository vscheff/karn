TIP_LIST = [
"To view all available commands, use the command: `$help`.",
...
"Want a random line from one of your server's text files? Simply append the \"#\" character to the front of a text file's name in a message, and I will send a random line from that file. Your message can contain multiple #filenames, and I will replace all of them with lines from their respective files.",
...
"You can specify how I refer to myself within LLM responses using the `$tee` command to add lines to your server's \"descriptor\" file.. To add additional self descriptors for me, perform the command: `$tee descriptor Your descriptor here`.\nFor example, after executing the command `$tee descriptor a silly little robot`, I will occasionally refer to myself as \"a silly little robot\".",
